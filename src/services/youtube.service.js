...
export const handleCallback = async (code, userId) => {
  ...
  if (channelRecord) {
    ...
    if (previousOwnerId.toString() !== userId.toString()) {
      ...
      console.log(
        `🔄 Surgical Migration: Moving ${channelId} from ${previousOwner?.email} to ${currentUser.email}`,
      );
      ...
      if (currentUser.plan === "Free") {
        ...
        if (previousOwner) {
          ...
          if (
            previousOwner.activeChannel?.toString() ===
      ...
      await Video.updateMany(
        { videoId: { $exists: true }, user: previousOwnerId },
        { $set: { user: userId } },
      );
      await Comment.updateMany(
        { userId: previousOwnerId, videoId: { $exists: true } },
        { $set: { userId: userId } },
      );
...
const getChannelComments = async (userId, pageToken = "", videoId = null) => {
  ...
  if (!user || !user.activeChannel)
    throw new Error("No active channel found. Please select a channel.");

  const channel = await Channel.findById(user.activeChannel).select(
    "+youtubeRefreshToken",
  );
  if (!channel || !channel.youtubeRefreshToken)
    throw new Error("Active channel not connected to YouTube.");
      ...
      if (item.replies && item.replies.comments) {
        replies = item.replies.comments
          .map((reply) => ({
            ...
            isOwner:
              reply.snippet.authorChannelId.value === channel.youtubeChannelId,
          }))
          .reverse();
      }
...
export const getChannelVideos = async (
  ...
  if (!forceRefresh && !pageToken && channel.lastVideoSync) {
    ...
    if (timeDiff < 24 * 60 * 60 * 1000) {
      ...
      const cachedVideos = await Video.find({ channel: channel._id })
        ...
        .limit(limit);
      if (cachedVideos.length > 0)
        return { videos: cachedVideos, source: "cache" };
    }
  }
  ...
  try {
    ...
    if (forceRefresh) {
      console.log(
        `🔄 Starting Full Sync for Channel: ${channel.youtubeChannelName}`,
      );
    ...
    const validItems = allFetchedVideos.filter(
      (item) => item.status.privacyStatus !== "private",
    );
    const fetchedVideoIds = validItems.map(
    ...
    // 6. RETURN DATA
    const freshVideos = await Video.find({ channel: channel._id })
      .sort({ publishedAt: -1 })
      .limit(limit);
...
export const postReplyToComment = async (userId, commentId, replyText) => {
  ...
  if (!channel || !channel.youtubeRefreshToken)
    throw new Error("Channel authorization missing.");
...
// export const getSmartComments = async (
//   user,
//   videoId,
//   pageToken = "",
//   refresh,
...
//   if (!fullUser || !fullUser.activeChannel)
...
//   }
//   const channel = await Channel.findById(fullUser.activeChannel).select(
//     "+youtubeRefreshToken",
//   );
//   if (!channel || !channel.youtubeRefreshToken)
//     throw new Error("Channel token missing.");
...
//   if (shouldFetchFromYouTube) {
//     console.log(
//       `🌐 Fetching from YouTube [${channel.youtubeChannelName}]. Reason: ${pageToken ? "Pagination" : refresh ? "Manual Refresh" : "Initial Sync"}`,
//     );
...
//             currentCommentReplies = item.replies.comments.map((r) => {
//               const isOwner =
//                 r.snippet.authorChannelId.value === channel.youtubeChannelId;
//               if (isOwner) ownerRepliedInThisThread = true;
...
export const getSmartComments = async (user, videoId, pageToken = '', forceRefresh = false) => {
    // 1. Pehle user ka active channel dhoondein
    const fullUser = await User.findById(user._id);
    if (!fullUser || !fullUser.activeChannel) {
        throw new Error('No active channel selected. Please switch to a channel first.');
    }

    // 2. Channel document fetch karein (with Refresh Token)
    const channel = await Channel.findById(fullUser.activeChannel).select('+youtubeRefreshToken');
    if (!channel || !channel.youtubeRefreshToken) {
        throw new Error('Active channel is not authorized. Please reconnect.');
    }

    // 3. Check current DB state for this specific video AND channel
    const dbCount = await Comment.countDocuments({ videoId, channel: channel._id });

    // YouTube API Trigger Logic
    const shouldFetchFromYouTube = dbCount === 0 || pageToken || forceRefresh;
    
    let ytNextPageToken = null;

    if (shouldFetchFromYouTube) {
        console.log(`🌐 Syncing YouTube for Channel: [${channel.youtubeChannelName}] | Reason: ${forceRefresh ? 'DEEP_SYNC' : 'PAGINATION'}`);
        ...
        // Deep Sync handle karega up to 5 pages (500 comments) agar manual refresh ho
        const MAX_PAGES = forceRefresh ? 5 : 1; 
        ...
        while (hasMorePages && pagesFetched < MAX_PAGES) {
            try {
                ...
                if (ytItems.length > 0) {
                    const ops = ytItems.map(item => {
                        const topComment = item.snippet.topLevelComment.snippet;
                        
                        // Extract Replies & Check Owner
                        let threadReplies = [];
                        let ownerReplied = false;

                        if (item.replies && item.replies.comments) {
                            threadReplies = item.replies.comments.map(r => {
                        ...
                        return {
                            updateOne: {
                                ...
                                update: {
                                    $set: {
                                        userId: fullUser._id,
                                        channel: channel._id, // 🔥 IMPORTANT: Link to Channel ObjectId
                                        videoId: videoId,
                                        authorName: topComment.authorDisplayName,
                                        authorAvatar: topComment.authorProfileImageUrl,
                                        text: topComment.textDisplay,
                                        replies: threadReplies,
                                        status: ownerReplied ? "Replied" : "Pending",
                                        publishedAt: topComment.publishedAt,
                                        lastSyncedAt: new Date()
                                    }
                                    // ❌ note: hum yahan 'draft' ya 'isFlagged' ko touch nahi kar rahe taake AI ka kaam save rahay
                                },
                                upsert: true
                            }
                        };
                ...
                currentToken = ytNextPageToken;
                pagesFetched++;
                if (!currentToken || !forceRefresh) hasMorePages = false;

            } catch (apiError) {
                console.error("YouTube Sync Error:", apiError.message);
                if (apiError.message.includes('disabled comments')) return { comments: [], nextPageToken: null };
                hasMorePages = false;
            }
        }

        // 🔥 SAVE TOKEN TO VIDEO: Store the token specifically for this channel's video
        await Video.findOneAndUpdate(
            { videoId, channel: channel._id }, 
            { nextPageToken: ytNextPageToken },
            { upsert: true }
        ...
        console.log(`⚡ Serving from DB Cache for Channel: ${channel.youtubeChannelName}`);
        // DB se saved token uthayen taake "Load More" chalta rahe
        const videoMeta = await Video.findOne({ videoId, channel: channel._id });
        ytNextPageToken = videoMeta?.nextPageToken;
    }

    // 4. FINAL RETURN (Pending First, then Newest First)
    // Filter by BOTH videoId and channel._id for strict isolation
    const comments = await Comment.find({ videoId, channel: channel._id })
        .sort({ status: 1, publishedAt: -1 });
